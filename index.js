...
class Review_Data {
    ...
    */
    ...
    static post = (database,parent_table,parent_id,user_table,user_id,post_review,option) => {
        return new Promise((callback) => {
            ...
            async.series([
                ...
                async function(call){
                    ...
                    response.messages.push(Response_Logic.get_message(Review_Response_Field.RESPONSE_REVIEW,Status_Type.OK,data.review,{title:Config.TITLE}));
                },
                async function(call){
                    ...
                    response.messages.push(Response_Logic.get_message(Review_Response_Field.RESPONSE_PARENT,Status_Type.OK,data.parent,{title:Config.TITLE}));
                },
	            async function(call){
                    // -- get review --
                ...
                async function(call){
                    ...
                    if(!Str.check_is_null(data.review.id)){
                        response.messages.push(Response_Logic.get_message(Response_Field.POST_CONFIRM,Status_Type.SUCCESS,Review_Logic.get_message_by_response_field(Response_Field.POST_CONFIRM),{title:Config.TITLE}));
                    }else{
                        response.messages.push(Response_Logic.get_message(Response_Field.POST_FAIL,Status_Type.SUCCESS,Review_Logic.get_message_by_response_field(Response_Field.POST_FAIL),{title:Config.TITLE}));
                    }
		 ...
		 let response=Response_Logic.get();
			...
			async.series([
				...
				async function(call){
                    ...
                    data = biz_data;
                    response.messages.push(Response_Logic.get_message(Review_Response_Field.RESPONSE_PARENT_SEARCH,Status_Type.OK,biz_response,{title:Config.TITLE}));
                    response.messages.push(Response_Logic.get_message(Review_Response_Field.RESPONSE_PARENT_SEARCH_ITEM_COUNT,Status_Type.OK,data.items.length,{title:Config.TITLE}));
				},
                //check all
                async function(call){
                    if(data.items){
                        response.messages.push(Response_Logic.get_message(Review_Response_Field.PARENT_SEARCH_CONFIRM,Status_Type.SUCCESS,Review_Logic.get_message_by_response_field(Review_Response_Field.PARENT_SEARCH_CONFIRM),{title:Config.TITLE}));
                    }else{
                        response.messages.push(Response_Logic.get_message(Review_Response_Field.PARENT_SEARCH_FAIL,Status_Type.SUCCESS,Review_Logic.get_message_by_response_field(Review_Response_Field.PARENT_SEARCH_FAIL),{title:Config.TITLE}));
                    }
                    response = Response_Logic.get_status(response);
                },
			]).then(result => {
    ...
    static delete = (database,parent_table,parent_id,review_id) => {
        return new Promise((callback) => {
            let response=Response_Logic.get();
            ...
            async.series([
                ...
                async function(call){
                    ...
                    if(!Str.check_is_null(biz_response.status == Status_Type.SUCCESS)){
                        response.messages.push(Response_Logic.get_message(Response_Field.DELETE_CONFIRM,Status_Type.SUCCESS,Review_Logic.get_message_by_response_field(Response_Field.DELETE_CONFIRM),{title:Config.TITLE}));
                    }else{
                        response.messages.push(Response_Logic.get_message(Response_Field.DELETE_FAIL,Status_Type.FAIL,Review_Logic.get_message_by_response_field(Response_Field.DELETE_FAIL),{title:Config.TITLE}));
                    }
                    response = Response_Logic.get_status(response);
                },
                async function(call){
                    const [biz_response,biz_data] = await Review_Data.caculate(database,parent_table,parent_id);
                    data.parent = biz_data;
                    response.messages.push(Response_Logic.get_message(Review_Response_Field.RESPONSE_CACULATE,Status_Type.OK,biz_response,{title:Config.TITLE}));
                },
            ]).then(result => {
                callback([response,data]);
            }).catch(err => {
