        ...
        return new Promise((callback) => {
            ...
            async.series([
                async function(call) {
                    ...
                    response.messages.push(Response_Logic.get_message(Data_Response_Field.PARAM_PARENT_TABLE,Status_Type.OK,parent_table,{title:BIZ9_CONFIG.TITLE}));
                ...
                async function(call){
                    ...
                    if(!Str.check_is_null(data.review.id)){
                        ...
                        response.messages.push(Response_Logic.get_message(Response_Field.POST_FAIL,Status_Type.SUCCESS,Review_Logic.get_message_by_response_field(Response_Field.POST_FAIL,{title:BIZ9_CONFIG.TITLE})));
                    }
		...
		return new Promise((callback) => {
		 let response=Response_Logic.get();
			...
			async.series([
                ...
                async function(call){
                    if(data.items){
                        response.messages.push(Response_Logic.get_message(Review_Response_Field.PARENT_SEARCH_CONFIRM,Status_Type.SUCCESS,Review_Logic.get_message_by_response_field(Review_Response_Field.PARENT_SEARCH_CONFIRM,{title:BIZ9_CONFIG.TITLE})));
                    }else{
                        response.messages.push(Response_Logic.get_message(Review_Response_Field.PARENT_SEARCH_FAIL,Status_Type.SUCCESS,Review_Logic.get_message_by_response_field(Review_Response_Field.PARENT_SEARCH_FAIL,{title:BIZ9_CONFIG.TITLE})));
                    }
                    response = Response_Logic.get_status(response);
                },
			]).then(result => {
    ...
    static delete = async(database,parent_table,parent_id,review_id) => {
        return new Promise((callback) => {
            ...
            async.series([
                async function(call) {
                    response.messages.push(Response_Logic.get_message(Data_Response_Field.PARAM_APP_ID,Status_Type.OK,database.data_config.APP_ID,{title:BIZ9_CONFIG.TITLE}));
                    response.messages.push(Response_Logic.get_message(Data_Response_Field.PARAM_PARENT_TABLE,Status_Type.OK,parent_tableD,{title:BIZ9_CONFIG.TITLE}));
                    response.messages.push(Response_Logic.get_message(Data_Response_Field.PARAM_PARENT_ID,Status_Type.OK,parent_id,{title:BIZ9_CONFIG.TITLE}));
                    response.messages.push(Response_Logic.get_message(Review_Response_Field.PARAM_REVIEW_ID,Status_Type.OK,review_id,{title:BIZ9_CONFIG.TITLE}));
                },
                //review_delete
                async function(call){
                    ...
                    if(!Str.check_is_null(biz_response.status == Status_Type.SUCCESS)){
                        response.messages.push(Response_Logic.get_message(Response_Field.DELETE_CONFIRM,Status_Type.SUCCESS,Review_Logic.get_message_by_response_field(Response_Field.DELETE_CONFIRM,{title:BIZ9_CONFIG.TITLE})));
                    }else{
                        response.messages.push(Response_Logic.get_message(Response_Field.DELETE_FAIL,Status_Type.FAIL,Review_Logic.get_message_by_response_field(Response_Field.DELETE_FAIL,{title:BIZ9_CONFIG.TITLE})));
                    }
                    response = Response_Logic.get_status(response);
                },
                async function(call){
